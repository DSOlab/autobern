...
def make_rinex2_fn(mark_name_dso, mark_name_off, pt):
    ...
    if mark_name_dso.lower() in ['noa1', 'tuc2', 'duth', 'aut1', 'pat0', 'larm']:
        ...
        lname = '{:}{:}0.{:}d.Z'.format(mark_name_dso, pt.strftime('%j'), pt.strftime('%y'))
        possible_rinex_fn = add2possible_rinex(possible_rinex_fn, mark_name_dso, rname, lname)
            ...
            if key not in dict2:
                ...
                if dict1[key] != dict2[key]:
                    if key not in thispair_difs:
                        thispair_difs.append(key)
                        verboseprint('[WRNNG] Different values for key {:}; values are: {:} and {:}'.format(key, dict1[key],dict2[key]))
...
def query_station(cursor, station, pt, holdings, output_dir=os.getcwd(), bearer_token=None):
    ...
    rows = [dict(row.items()) for row in cursor.fetchall()]
    if not rows or rows == {}:
...
def main(**kwargs):
    ...
    for k,v in zip(['username', 'password', 'db_host', 'mysql_host', 'db_name'], ['GNSS_DB_USER', 'GNSS_DB_PASS', 'GNSS_DB_HOST', 'GNSS_DB_HOST', 'GNSS_DB_NAME']):
        if k in kwargs and kwargs[k] is not None:
            credentials_dct[v] = kwargs[k]
