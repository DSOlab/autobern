...
class Ionex:
  ...
  def __init__(self, filename):
    ...
    if not self.check_format():
      msg = '[ERROR] Failed to find/validate IONEX file: {:}'.format(self.filename)
  ...
  def read_header(self):
    with open(self.filename, 'r') as f:
      for line in f.readlines():
        if line[60:].strip() == 'END OF HEADER': break
        if line[60:].strip() == 'PGM / RUN BY / DATE':
          ...
          self.epoch_of_first_map = datetime.datetime.strptime(line[0:37].strip(), '%Y %m %d %H %M %S')
        elif line[60:].strip() == 'EPOCH OF LAST MAP':
          self.epoch_of_last_map = datetime.datetime.strptime(line[0:37].strip(), '%Y %m %d %H %M %S')
