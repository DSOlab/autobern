...
def parse_key_file(fn, parse_error_is_fatal=False):
  result = {}
  lnrgx = re.compile('^\s*(?P<var_name>\w+)\s*=\s*"?(?P<var_value>[ a-zA-Z0-9!@#\$%\^\&*\)\(+=._-]+)"?\s*$')
  with open(fn, 'r') as f:
    for line in f.readlines():
      if not line.startswith('#'):
        m=re.match(lnrgx, line.strip())
        if not m and len(line.strip())>0:
          print('[WRNNG] Line is not a comment but could not be resolved: \'{:}\''.format(line.strip()), file=sys.stderr)
          if parse_error_is_fatal:
            msg = '[ERROR] keyholders::parse_key_file unable to parse line: \'{:}\''.format(line.strip())
