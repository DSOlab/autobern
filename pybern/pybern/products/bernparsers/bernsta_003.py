...
class Type003Record:
    ...
    def init_from_line(self, line):
      ...
      if len(self.sta_name) < 4:
        raise FileFormatError(FILE_FORMAT, line, '[ERROR] Type003Record::init_from_line Failed to parse station name')
      self.flag = line[22:25].rstrip()
        ...
        if len(t_str) == 0:
          ...
          try:
            self.start_date = datetime.datetime.strptime(t_str.strip(), '%Y %m %d %H %M %S')
          except:
            raise FileFormatError(FILE_FORMAT, line, '[ERROR] Type003Record::init_from_line Failed to parse start date')
        ...
        if len(t_str) == 0:
          ...
          try:
            self.stop_date = datetime.datetime.strptime(t_str.strip(), '%Y %m %d %H %M %S')
          except:
            raise FileFormatError(FILE_FORMAT, line, '[ERROR] Type003Record::init_from_line Failed to parse stop date')
    ...
    def __str_format__(self):
      ...
      return '{:-16s}      {:03d}  {:}  {:} {:-20s}  {:-20s}  {:-s}'.format(self.sta_name, self.flag, t_start, t_stop, self.remark)
