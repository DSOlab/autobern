...
class StationRecord:
    ...
    def compact_str(self):
        ...
        for t002 in self.t002rec:
            str_ += '\n{:}'.format(t002)
        return str_

    def add_002_record(t002rec):
        if t002rec.sta_name != self.t001rec.sta_name:
            raise StationInformationError(
                self.t001rec.sta_name,
                '[ERROR] StationRecord::add_002_record Station name does not match',
                self, t002rec)
        if t002rec.start_date < self.t001rec.start_date or t002rec.stop_date > self.t001rec.stop_date:
            raise StationInformationError(
                self.t001rec.sta_name,
                '[ERROR] StationRecord::add_002_record Type002 info has invalid date bounds (#1)',
                self, t002rec)
        for rec in self.t002rec:
            if date_ranges_overlap(t002rec.start_date, t002rec.stop_date,
                                   rec.start_date, rec.stop_date):
                raise StationInformationError(
                    self.t001rec.sta_name,
                    '[ERROR] StationRecord::add_002_record Time overlap in new Type002 record',
                    self, t002rec)
    ...
    def add_003_record(t003rec):
        if t003rec.sta_name != self.t001rec.sta_name:
            raise StationInformationError(
                self.t001rec.sta_name,
                '[ERROR] StationRecord::add_003_record Station name does not match',
                self, t003rec)
        if t003rec.start_date < self.t001rec.start_date or t003rec.stop_date > self.t001rec.stop_date:
            raise StationInformationError(
                self.t001rec.sta_name,
                '[ERROR] StationRecord::add_003_record Type003 info has invalid date bounds (#1)',
                self, t003rec)
