...
def validate_interval(pydt, filename, informat):
  dct = {'compressed': is_compressed(filename), 'ctype': find_os_compression_type(filename)}
  ## if file is compressed, decompress first
  filename, decomp_filename = dc.os_decompress(filename)
  status = 0
  #try:
  ion = BernIon(decomp_filename) if informat=='bernese' else Ionex(decomp_filename)
  fstart, fstop = ion.time_span()
  dstart, dstop = pydt, pydt+datetime.timedelta(seconds=86400)
  if dstart < fstart or dstop > fstop: status = 10
  print('Validation: File  start epoch: {:} stop epoch {:}'.format(fstart.strftime('%Y-%m-%d %H:%M:%S'), fstop.strftime('%Y-%m-%d %H:%M:%S')))
  print('Validation: Given start epoch: {:} stop epoch {:}'.format(dstart.strftime('%Y-%m-%d %H:%M:%S'), dstop.strftime('%Y-%m-%d %H:%M:%S')))
  if dct['compressed']: cc.os_compress(decomp_filename, dct['ctype'], True)
  return status
  #except:
  #  sys.exit(5)
...
if __name__ == '__main__':
    ...
    if args.code_euref: input_dct['acid'] = 'coe'
    if args.save_as: input_dct['save_as'] = args.save_as
    if args.save_dir: input_dct['save_dir'] = args.save_dir
        ...
        if not status:
          ...
          if args.validate_interval:
            ...
            if not j:
              ...
              print('Ionospheric file {:} does not include the correct interval!')
          else:
            sys.exit(0)
